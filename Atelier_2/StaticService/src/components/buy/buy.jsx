...
export const BuyPage = () => {
  ...
  const fetchShopCards = async () => {
    try {
      const response = await fetch('http://localhost/superservice/cards_to_sell');
      if (!response.ok) {
  ...
  const handleBuyClick = async (card) => {
    try {
      ...
      if (responseData === true) {
        ...
        setShopCards(prevShopCards => prevShopCards.filter(shopCard => shopCard.id !== card.id));
        setCurrentUserMoney(prevMoney => prevMoney - card.price);
      } else {
      ...
      <div className="ui clearing segment">
        <h3 className="ui right floated header">
          ...
          <div className="content">
            ...
            <div className="sub header"><span>{currentUserMoney}</span>$</div>
          </div>
        </h3>
