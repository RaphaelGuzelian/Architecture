...
public class CardRestController {
	...
	public List<Integer> getRandCard(@RequestParam int quantity, @RequestParam String userId) {
	...
	private List<CardDTO> getCardsToSell() {
		...
		System.out.println("[CARDSERVICE] : Toutes les cartes sans propriétaire : "+list);
		return list;
	...
	private List<CardDTO> getCardsByUserId(@PathVariable String userId) {
		System.out.println("[CARDCONTROLLER] : Toutes les cartes de l'utilisateur : "+cardModelService.getCardsByUserId(userId));
		return cardModelService.getCardsByUserId(userId);
	}
